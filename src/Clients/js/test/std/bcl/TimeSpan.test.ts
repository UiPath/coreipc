...
describe(`${TimeSpan.name}'s`, () => {
    ...
    describe(`the isNonNegative property`, () => {
        it(`shouldn't throw`, () => {
            for (const _case of _cases.filter(x => !x.isInvalid)) {
                const millisecondCount = _case.arg as any as number;
                const timeSpan = TimeSpan.fromMilliseconds(millisecondCount);
    ...
    describe(`📞 "bind" method`, () => {
        it(`should cause a ${PromiseCompletionSource.name} to complete in the faulted state with a ${TimeoutError.name}`, async () => {
            const pcs = new PromiseCompletionSource<boolean>();

            TimeSpan.fromMilliseconds(10).bind(pcs);
            ...
            expect(caught).to.be.instanceOf(TimeoutError);
        });

        it(`should synchronously cause a ${PromiseCompletionSource.name} to complete in the faulted state with a ${TimeoutError.name} when the ${TimeSpan.name} is zero`, async () => {
            const pcs = new PromiseCompletionSource<boolean>();

            TimeSpan.zero.bind(pcs);
