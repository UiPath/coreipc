...
export class CancellationTokenSource implements IDisposable {
    public static createLinkedTokenSource(...tokens: CancellationToken[]): CancellationTokenSource {
        ...
        if (tokens.filter(x => !(x instanceof CancellationToken)).length > 0) {
            throw new ArgumentError('Some supplied arguments were not instances of CancellationToken.');
        }
        ...
        const disposable = new AggregateDisposable(
            ...tokens.map(
                token => token.register(onCancellation)));
    ...
    constructor(arg0?: number | TimeSpan) {
        ...
        if (arg0 != null) { arg0 = TimeSpan.toTimeSpan(arg0); }

        if (arg0 && !arg0.isInfinite) {
            if (arg0.isNegative) { throw new ArgumentOutOfRangeError(paramName, 'Specified argument represented a negative interval.'); }
    ...
    public cancel(throwOnFirstError: boolean = false): void {
        argumentIs(throwOnFirstError, 'throwOnFirstError', 'boolean');
    ...
    /* @internal */ public get isCancellationRequested(): boolean { return this._isCancellationRequested; }
    ...
    private assertNotDisposed(): void {
        if (this._isDisposed) {
            throw new ObjectDisposedError(undefined, 'The CancellationTokenSource has been disposed.');
        }
    }
