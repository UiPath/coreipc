...
describe(`foundation:threading -> class:TimeSpan`, () => {
    ...
    context(`property:isNonNegative`, () => {
        it(`shouldn't throw`, () => {
            for (const _case of _cases.filter(x => !x.isInvalid)) {
                const millisecondCount = _case.arg as any as number;
                const timeSpan = TimeSpan.fromMilliseconds(millisecondCount);
    ...
    context(`method:toString and calling JSON.stringify on a TimeSpan`, () => {
        it(`shouldn't throw`, () => {
            for (const _case of _cases.filter(x => !x.isInvalid)) {
                const millisecondCount = _case.arg as any as number;
                const timeSpan = TimeSpan.fromMilliseconds(millisecondCount);
